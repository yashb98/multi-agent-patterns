...
# ---------------------------------------------------------------------------
# Module-level helper — does NOT depend on ScreeningPipeline instance
# ---------------------------------------------------------------------------
...
def query_memory_for_similar_answer(
    ...
    jd_context: str = "",
    *,
    min_decay_score: float = 0.7,
) -> str | None:
    """Cross-domain answer fallback for novel screening questions.

    Searches the 3-engine memory stack for similar past answers when this
    domain has no cached answer.  Returns the best match's answer if its
    decay_score >= min_decay_score, else None.

    NOTE: min_decay_score is a *freshness/activity floor*, not a cosine
    similarity threshold.  MemoryManager.query() sorts results by decay_score
    (recency + access-count signal from ForgettingEngine), discarding
    Qdrant's per-result similarity.  Tune min_decay_score to control how
    "stale" an answer is allowed to be, not how semantically close it is.

    Storage convention: entries are stored as "screening_answer: <answer>",
    so the leading "screening_answer: " prefix is stripped before returning.

    Uses MemoryQuery(semantic_query=...) so the QueryRouter picks the best
    engine(s) available — Qdrant vector search when configured, FTS fallback
    otherwise.  If the MemoryManager has no SQLite store attached (dev /
    test default), query() returns [] immediately with no I/O.
    """
    search_text = f"screening_answer: {question}"
    if jd_context:
        search_text = f"{search_text} context: {jd_context[:200]}"

    try:
        from shared.memory_layer import MemoryQuery
    ...
    # query() already sorts by decay_score desc; results[0] is the best match.
    best = results[0]
    score = best.decay_score if best.decay_score is not None else 0.0
    if score < min_decay_score:
        return None
    ...
    # Strip the "screening_answer: " tag prefix so callers get a clean string.
    if ":" in content:
