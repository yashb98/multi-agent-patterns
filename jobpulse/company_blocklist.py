...
def create_blocklist_database(parent_page_id: str | None = None) -> dict:
    """Create the Notion blocklist database with the required schema.

    Args:
        parent_page_id: Notion page ID to create the DB under.
                        Falls back to NOTION_PARENT_PAGE_ID env var.

    Returns:
        Notion API response dict (includes the new DB ID).
    """
    from jobpulse.config import NOTION_PARENT_PAGE_ID

    page_id = parent_page_id or NOTION_PARENT_PAGE_ID
    if not page_id:
        logger.error("No parent page ID — cannot create blocklist database")
        return {}

    payload = {
        "parent": {"page_id": page_id},
        "title": [{"text": {"content": "Company Blocklist"}}],
        "properties": {
            "Company": {"title": {}},
            "Status": {
                "select": {
    ...
    if resp.get("id"):
        logger.info("Created blocklist database: %s", resp["id"])
    else:
        logger.error("Failed to create blocklist database: %s", resp)
    return resp
