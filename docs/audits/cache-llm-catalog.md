...
| Status | File:Line | Function | Class | Routing | Cache key / DB source | Notes |
|---|---|---|---|---|---|---|
| 🟡 | `jobpulse/cv_tailor.py:252` | `_call_with_correction` | **CACHE-REPLACEABLE** | `cognitive_llm_call` | NEW table `cv_tailor_cache` keyed by `(role_archetype, jd_hash, profile_version)` | §2.2 #1 — runs unconditionally per JD. Need `(role_archetype, jd_hash, profile_version)` cache. |
| 🟡 | `jobpulse/gate4_quality.py:254` | `scrutinize_cv_llm` | **CACHE-REPLACEABLE** | `cognitive_llm_call` | NEW `gate4_cache` *or* extend `cv_tailor_cache` keyed by `(cv_hash, jd_hash)` | Phase B1/B2 LLM CV scrutiny; deterministic given same inputs. |
