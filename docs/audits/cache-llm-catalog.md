...
| ✅ S7 0a3cfda | `jobpulse/strategy_reflector.py:182` | `reflect_with_llm` | **NECESSARY** (cognitive-routed) | `cognitive_llm_call` | Migrated from `smart_llm_call` (which bypassed cognitive engine entirely) to `cognitive_llm_call(domain="strategy_reflection", stakes="medium")`. Now flows through L0 Memory Recall → L1 Single-Shot → L2 Reflexion → L3 ToT escalation. Drop the explicit `track_llm_usage` call — cognitive engine's own infra handles cost tracking. Markdown-fence stripping retained so cognitive-engine outputs that wrap JSON in ` ``` ` parse without a retry. Existing tests in `test_strategy_reflector.py::TestReflectWithLLM` rewritten to patch `cognitive_llm_call` (5 tests; all 5 fail under stash-drill confirming the migration is what makes them green). |
...
| ✅ S7-EXT f6ced0d | `jobpulse/gmail_agent.py:102` | `_classify_email` | **CACHE-REPLACEABLE** (cognitive-routed) | `cognitive_llm_call` | Migrated post-S7 in commit `f6ced0d`: replaced the manual `engine.think_sync` → `smart_llm_call` fallback chain with a single `cognitive_llm_call(domain="email_classification", stakes="medium", scorer=_score_classification)` call. Cognitive engine's L0 Memory Recall now applies to repeat senders / repeat templates. Evolved-prompt context (`get_evolved_prompt("gmail_agent")`) is now inlined upstream so it applies consistently across cognitive and direct paths. Tests: `test_gmail_agent_real.py::TestClassifyEmailCognitiveRouting` (4 tests; load-bearing test asserts cognitive_llm_call invoked with right domain + scorer; fails under stash-drill). |
