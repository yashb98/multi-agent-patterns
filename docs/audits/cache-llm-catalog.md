...
| 🟡 S7-EXT | `jobpulse/gmail_agent.py:102` | `_classify_email` | **CACHE-REPLACEABLE** | `smart_llm_call` | Gmail recruiter classifier fallback. **Bypasses cognitive.** Deferred from S7 — keeps cluster scope at ≤2 subsystems per `cache-or-llm-audit.md §7`. Same migration shape as `strategy_reflector` (drop `get_llm` + `smart_llm_call`, swap for `cognitive_llm_call(domain="email_classification", ...)`). |
