...
| ✅ S7 ef7ae66 | `shared/memory_layer/_stores.py:SemanticMemory.learn → _evict_to_cap` | `SemanticMemory.learn` now evicts to `max_facts` after appending a new fact: sort by `reliability * confidence`, tiebreak `times_validated`, keep top N. Mirrors `EpisodicMemory.store` and `ProceduralMemory.store`. Production `semantic.json` will converge to ≤500 entries on the next reinforcement cycle. |
| ✅ S7 ef7ae66 | `shared/memory_layer/_manager.py:get_procedural_entries / get_episodic_entries / get_semantic_entries` | All three accessors are now SQLite-first with JSON fallback. Procedural reads use a single windowed SQL query (`aggregate_procedural_by_strategy`) that dedups by `SUBSTR(content,1,50)` and aggregates `times_used` / `avg_score` / `avg_success_rate` — collapsing the 19 783 `optimization_success_streak` write-amplified rows into ~378 distinct strategies as cognitive expects. **Behavior change**: `EscalationClassifier` will hit `L0_MEMORY` more often because high-volume domains finally meet `times_used>=3 AND avg_score>=8`. |
...
| 🩹 S11 M-11.A | `shared/memory_layer/_linker.py` | `AutonomousLinker.link_with_neighbors` is implemented but never invoked from production. Effect is identical to "missing code" — 3 of 6 forgetting signals always return defaults. | **FIXED** in `7409548` (S6) — `SyncService._sync_entry` now calls `_link_neighbors` after upsert/node creation. Pattern-test `tests/shared/memory_layer/test_linker_wiring.py` spies on `link_with_neighbors` and asserts non-empty neighbor invocation. |
...
| ✅ S6 7409548 | `_linker.py` whole module (apply path) | See S11 M-11.A. | **Verdict: WIRED.** S6 invokes `AutonomousLinker.link_with_neighbors` from `SyncService._sync_entry`. `handle_contradiction` remains unwired (semantic-fact contradiction detection deferred to a later session — runner table S6 acceptance was `link_with_neighbors` only). |
...
| ✅ S7 ef7ae66 | `get_procedural_entries`/`get_episodic_entries` read JSON; `query` reads SQLite — **same store, divergent reads** (M-11.C). | **Verdict: WIRED.** Both accessors now read SQLite first; JSON fallback only for backwards compat (no `sqlite_store` configured) or when SQLite is empty for the domain. |
| ✅ S7 ef7ae66 | `cognitive/_classifier.py:179` reaches into `self._memory.semantic.facts.items()` directly — bypasses `MemoryManager` (S6 W-2 carryover). | **Verdict: WIRED.** S7 added `get_semantic_entries` accessor and migrated `load_persisted_stats` to use it. |

### S12 — ats_adapters

| ID | Description |
|---|---|
| 🔌 DEFERRED→S11 | `_cv_pre_uploaded` write-only flag (see Section 1). | **Verdict: DEFERRED to runner-table session S11 (verification readback sweep).** S11 wires `file_uploader` to read the flag, closing the double-CV-upload risk. |
...
4. **Wired-but-unconsumed infrastructure** — S5 M-5.3 (heuristic replay),
   ...
   S11 M-11.A (AutonomousLinker), S12 W-12.1 (`_cv_pre_uploaded`). Resolved in
   `pipeline-bugs-S5` (wire-or-delete decisions): heuristic replay,
   cross-platform transfer, PRAXIS-aware form experience, persona_drift,
   gate_decisions writer, memory_access_log writer, and the `transfer`
   signal type all marked DELETE; `_linker.py` deferred to S6 (linker
   wiring); `_cv_pre_uploaded` deferred to S11 (verification readback);
   `pin_memory` half-applied marked WIRE; `JOBPULSE_ARCHETYPE_ENGINE`
   default flip marked WIRE. Implementations land in follow-up dead-code /
   wiring sessions. Tripwire for the `_cv_pre_uploaded`-shape regression:
   `tests/lint/test_no_write_only_flag.py` (added in `pipeline-bugs-S2`) —
   guards `jobpulse/ats_adapters/` + `jobpulse/form_engine/`. Audit-table
   completeness tripwire: `tests/lint/test_pipeline_bugs_audit_complete.py`
   (added in `pipeline-bugs-S5`) — fails if any §3-5 row lacks a verdict.
