...
| ID | Location | Description |
|---|---|---|
| 💀 d-1 | `native_form_filler.py:3193-3211` | `scan_current_values` — public method, **0 callers**. Accesses `f["locator"]` which the field scanner doesn't always populate. |

### S3 — navigation

| ID | Location | Description |
|---|---|---|
| 💀 d-1 | `_navigator.py:1785-1824` | `verify_submission` (also S3 M-B). |
| 💀 d-2 | `overlay_dismisser.py` | `_dismiss_cookie_banner / _generic_modal / _promo_popup` — legacy `cookie_dismisser.dismiss` runs instead. |
| 💀 d-3 | `account_manager.py` | After 2026-05-04 auth rewrite, only `mark_verified` is reachable. `create_account / get_credentials / get_account_info / list_accounts` unused. |
| 💀 d-4 | `verification_detector.py` | Module unused in apply path; `playwright_driver.get_snapshot` does the equivalent inline. |

### S4 — screening_pipeline

| ID | Location | Description |
|---|---|---|
| 💀 d-1 | `screening_detector.py` | Zero production callers. |
| 💀 d-2 | `screening_pattern_extractor.py` | `extract_patterns` and `find_matching_pattern` write to clusters DB, but reads C/D-tier. |
| 💀 d-3 | `screening_pipeline.record_outcome` | Not called in apply path. |
...
| ID | Location | Description |
|---|---|---|
| 💀 d-5.1 | `correction_capture.py:160-220` | `get_correction_count`, `get_correction_rate`, `get_high_correction_fields` (test-only). |
| 💀 d-5.2 | `agent_rules.py:346-349` | `get_escalation_fields` (also S5 M-5.2). |
| 💀 d-5.3 | `cross_platform_field_transfer.py` (whole module) | No production importer; only tests + `weekly_optimize.py`. |
| 💀 d-5.4 | `form_experience_db.py:377, 444, 902-958` | PRAXIS-aware subsystem. |
| 💀 d-5.5 | `platform_transfer.py:362-396` | `record_outcome` has no production caller despite signal-emit path now working post-S5 B-1. |
| 💀 d-5.6 | `trajectory_store.py:555-584, 714-787` | Heuristic-replay subsystem. |

### S6 — cognitive_engine

| ID | Location | Description |
|---|---|---|
| 💀 d-1 | `shared/cognitive/_strategy.py:165-170` | `StrategyComposer.record_template_outcome` — no production caller. |
| 💀 d-2 | `shared/cognitive/_engine.py:292` | `CognitiveEngine.report()` reachable only from tests + analytics. |
...
| ID | Location | Description |
|---|---|---|
| 💀 d-9.1 | `jobpulse/job_scanners/totaljobs.py` (whole module) | Removed from `PLATFORM_SCANNERS` 2026-05-04 (scripts/install_cron.py:47); only test imports it. |

### S10 — optimization_engine

| ID | Location | Description |
|---|---|---|
| 💀 d-10.1 | `shared/optimization/_policy.py:155 decide_async` | Only test calls it; production uses synchronous `decide`. Emits `cognitive_decision` action that has zero rows in production and zero handlers in `_execute_one`. |
| 💀 d-10.2 | `shared/optimization/_gate_policy.py` (whole module, 242 LOC) | Only `tests/shared/optimization/test_gate_policy.py` imports it. Not in `__init__.py`. `_discover_domains:190` uses hardcoded English-only keyword classification (Principle 8). |

### S11 — memory_layer

| ID | Location | Description |
|---|---|---|
| 💀 d-11.1 | `data/agent_memory/memory.db` (0 bytes) | Orphan file, no production code references it. |
| 💀 d-11.2 | `_linker.py` whole module (apply path) | See S11 M-11.A. |
| 💀 d-11.3 | `_router.py` `TieredRouter` | Constructed but never invoked from apply path. |
| 💀 d-11.4 | `_qdrant_store.py:213` `search_all_tiers` | No production caller. |
| 💀 d-11.5 | `_qdrant_store.py:240` `count` | Test/analytics only. |
| 💀 d-11.6 | `_sqlite_store.py:262, 271, 280, 289, 307` | `query_by_tier`, `query_by_domain`, `query_by_lifecycle`, `query_by_decay_desc`, `query_tombstoned_recent` — `MemoryManager.query` doesn't use them. |
| 💀 d-11.7 | `_stores.py` `ShortTermMemory` whole class | Pattern-tier only. |
| 💀 d-11.8 | `_pattern.py` whole module (apply path) | Pattern-tier only. |
...
| ID | Location | Description |
|---|---|---|
| 💀 D-12.1 | `BasePlatformStrategy` ABC | 5 virtual methods with **zero callers anywhere**: `apply_button_selectors`, `wait_for_form_hydrated_ms`, `iframe_names`, `custom_field_scan`, `field_fill_overrides`. |
| 💀 D-12.3 | `BaseATSAdapter` | `resolve_selector` (line 49) and `get_wait_override` (line 61) — zero callers. Existed for the deleted per-platform adapter classes; missed in 2026-04 unification. |
| 💀 D-12.4 | `__init__.py:34 reset_adapter` | Zero callers anywhere. |
| 💀 D-12.5 | `strategy.py:61 list_registered_strategies` | Zero callers anywhere. |
