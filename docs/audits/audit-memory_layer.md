...
- **🔴 M-A** `_manager.py:151` constructs `self._linker = AutonomousLinker(neo4j=neo4j)` but no production callsite ever invokes `_linker.link_with_neighbors(...)`. `store_memory` writes to SQLite + queues secondary sync; `_sync._sync_entry` upserts Qdrant + creates Neo4j node — but never calls the linker. Result: **Neo4j has Memory nodes but zero edges** in production. Cascade impact: `ForgettingEngine.compute_decay`'s connectivity / impact / uniqueness graph signals always return defaults (degree=0, downstream_score=0.0, count_similar=0) — half of the 6-signal decay formula is decorative for every entry. Same shape as S5 M-5.3 (heuristic-replay) and S5 M-5.4 (cross-platform transfer): wired-but-unconsumed infrastructure. **Why deferred:** correct fix requires per-write Qdrant top-K search + SQLite hydration + Neo4j round-trips inside the background worker — that's a behavioral change worth its own session with proper benchmarks, plus the dev env can't exercise the Neo4j edge creation today (auth misconfig, M-D). Ship: documented + tracked.
