...
- **✅ M-B (FIXED in pipeline-bugs S7)** `_stores.py:217-258` — pre-fix `SemanticMemory.learn` documented `max_facts=500` but never enforced it; production `semantic.json` had 1 041 entries. **Fix:** added `_evict_to_cap()` called on each new-fact insert; sort by `reliability * confidence` (tiebreak on `times_validated`) and keep top `max_facts`. Mirrors `EpisodicMemory.store` / `ProceduralMemory.store`. Reinforcement of an existing fact does not trigger eviction (only new fact creation does). Tests: `tests/shared/memory_layer/test_read_unification.py::TestSemanticMemoryEviction` (3 tests).
- **✅ M-C (FIXED in pipeline-bugs S7)** `_manager.py:get_procedural_entries / get_episodic_entries / get_semantic_entries` — all three accessors now read SQLite as the source of truth, with JSON fallback only when `sqlite_store=None` (legacy mode) or SQLite is empty for the queried domain. `get_procedural_entries` uses a single SQL window-function query (`SQLiteStore.aggregate_procedural_by_strategy`) that dedups by `SUBSTR(content,1,50)`, aggregates `times_used` / `avg_score` / `avg_success_rate` via `COUNT/AVG OVER (PARTITION BY ...)`, and picks the top-scoring representative row per group via `ROW_NUMBER() OVER (... ORDER BY score DESC) = 1`. One round-trip, no N+1. Reconstructs `ProceduralEntry` / `EpisodicEntry` / `SemanticEntry` from `MemoryEntry.payload + content` so cognitive consumers' typed signatures stay intact. **Behavior change:** `EscalationClassifier` will hit `L0_MEMORY` more often because high-volume domains finally meet the `times_used>=3 AND avg_score>=8 AND success_rate>=0.8` threshold (this is what cognitive was *supposed* to do — JSON cap was undercounting). The sibling W-11.5 (`_classifier.load_persisted_stats` reaching into `memory.semantic.facts`) closes alongside via the new `get_semantic_entries` accessor. Tests: `tests/shared/memory_layer/test_read_unification.py::TestProceduralReadUnification`, `TestEpisodicReadUnification`, `TestSemanticEntriesAccessor` (6 tests).
