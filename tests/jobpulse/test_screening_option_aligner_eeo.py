...
class TestNoRecursionOnFallback:
    ...
    def test_ambiguous_answer_does_not_recurse(self):
        ...
        # Bumped 50 → 80 at S3+S4 merge: S3 wires record_decision() into
        # OptionAligner.align_answer which adds ~5-10 stack frames per
        # call (sqlite3.connect + conn.execute). The test asserts no
        # *recursion*, not absolute frame depth — 80 keeps the recursion
        # signal while accommodating the new instrumentation.
